  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="grid grid-cols-[80px_1fr_100px_1fr_100px] px-5 py-2.5 bg-[#F8F9FF] text-[10px] text-slate-400 font-bold tracking-wider gap-2">
          <span>NGÀY</span><span>BÀI ĐĂNG</span><span>KÊNH</span><span>SEATALK STAT</span><span>WEB</span>
        </div>
        {sorted.map(p => (
          <div key={p.id} className={`grid grid-cols-[80px_1fr_100px_1fr_100px] px-5 py-3.5 border-t border-slate-50 items-center gap-2 text-xs ${p.status !== 'posted' ? 'bg-amber-50/40' : ''}`}>
            <span className="text-slate-400">{formatDateShort(p.scheduled_at)}</span>
            <span className={p.status === 'posted' ? 'font-medium' : 'text-slate-400'}>
              {p.title}{p.status !== 'posted' ? ' — chưa đăng' : ''}
            </span>
